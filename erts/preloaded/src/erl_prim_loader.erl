...
set_primary_archive(undefined, undefined, undefined, MFA) ->
    request({set_primary_archive, undefined, undefined, undefined, MFA});
set_primary_archive(File, ArchiveBin, FileInfo, MFA)
  when is_list(File), is_binary(ArchiveBin), is_record(FileInfo, file_info) ->
    request({set_primary_archive, File, ArchiveBin, FileInfo, MFA}).
...
handle_set_primary_archive(State= #state{loader = efile}, File, ArchiveBin, FileInfo, MFA) ->
    ?SAFE2(efile_set_primary_archive(State, File, ArchiveBin, FileInfo, MFA), State).
...
prim_set_primary_archive(PS, undefined, undefined, undefined, _MFA) ->
    ...
    case PS#prim_state.primary_archive of
        undefined ->
            ...
            {Res, PS};
        ArchiveFile ->
            {primary, PrimZip, _FI, _MFA2} = erase(ArchiveFile),
            ...
            undefined ->
                case load_prim_archive(ArchiveFile, ArchiveBin, FileInfo) of
                    {ok, PrimZip, FI, Ebins} ->
                        ...
                        put(ArchiveFile, {primary, PrimZip, FI, MFA}),
                        {{ok, Ebins},
...
apply_archive(PS, Fun, Acc, Archive) ->
    case get(Archive) of
		...
		{ok, FI2} ->
		    ...
		    case load_prim_archive(Archive, FI2, MFA) of
			{ok, PrimZip2, FI3, _Ebins} ->
			    debug(PS, {cache, {update, Archive}}),
			    put(Archive, {primary, PrimZip2, FI3, MFA});
			Error2 ->
...
load_prim_archive(ArchiveFile, FileInfo, {M,F,_A}) ->
    case M:F(ArchiveFile) of
	{ok, ArchiveBin} ->
